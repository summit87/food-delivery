...
public class MessageSender {
	...
	public <T, U> SettableListenableFuture send(T key, U value,
		...
		try {
			...
			send.addCallback(
				new ListenableFutureCallback<SendResult<T, U>>() {
					@Override
					...
					public void onSuccess(SendResult<T, U> result) {
						log.info(
							"Message sent on topic {} ,partition {},offset {}",
	...
	public <T, U> SettableListenableFuture send(
		...
		try {
			ListenableFuture<SendResult<T, U>> send = kafkaProducerTemplate.send(producerRecord);
