...
@Slf4j
public class AppEventSerialization
		extends JsonDeserializer<AppEvent> {

	@Override
	public AppEvent deserialize(JsonParser jsonParser,
		...
		try {
			if (node != null) {
				type = AppEvent.valueOf((node.textValue()));
				if (type != null) {
					return type;
				}
			}
