...
public class CustomerProfileServiceImpl implements CustomerProfileService {
  ...
  @Override
  public void saveCustomerProfile(CustomerDetails customerDetails, HttpHeaders headers)
      throws Exception {

    CustomerProfileEntity profileEntity =
        GenericBuilder.of(CustomerProfileEntity::new)
            .with(
                CustomerProfileEntity::setFirstName,
                customerDetails.getCustomerProfile().getName().getFirstName())
            .with(
                CustomerProfileEntity::setLastName,
                customerDetails.getCustomerProfile().getName().getLastName())
            .with(
                CustomerProfileEntity::setPassword, customerDetails.getUserSecrets().getPassword())
            .with(CustomerProfileEntity::setUserId, customerDetails.getUserSecrets().getUserId())
            .with(
                CustomerProfileEntity::setHouseNumber,
                customerDetails.getCustomerProfile().getCustomerAddress().getHouseNumber())
            .with(
                CustomerProfileEntity::setStreetName,
                customerDetails.getCustomerProfile().getCustomerAddress().getStreetName())
            .with(
                CustomerProfileEntity::setNearestLandMark,
                customerDetails.getCustomerProfile().getCustomerAddress().getNearestLandMark())
            .with(
                CustomerProfileEntity::setZipCode,
                customerDetails.getCustomerProfile().getCustomerAddress().getZipCode())
            .with(
                CustomerProfileEntity::setCountryCode,
                customerDetails.getCustomerProfile().getCustomerAddress().getCountryCode())
            .with(
                CustomerProfileEntity::setPrimaryMobileNumber,
                customerDetails.getCustomerProfile().getCustomerContact().getPrimaryMobileNumber())
            .with(
                CustomerProfileEntity::setSecondaryMobileNumber,
                customerDetails
                    .getCustomerProfile()
                    .getCustomerContact()
    ...
    log.info("User profile for user id {} created", customerDetails.getUserSecrets().getUserId());
  }
  ...
  public CustomerDetails findCustomerProfileByEmailId(String emailId) throws Exception {
    ...
    return GenericBuilder.of(CustomerDetails::new)
        .with(
            CustomerDetails::setCustomerProfile,
